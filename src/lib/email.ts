  ...
  if (process.env.BREVO_API_KEY) {
    try {
      const res = await fetch("https://api.brevo.com/v3/smtp/email", {
        ...
        body: JSON.stringify({
          sender: { name: "Hive", email: process.env.BREVO_SENDER_EMAIL || "hive.collab@gmail.com" },
          to: [{ email: to }],
          subject,
          htmlContent: html,
        }),
...
export async function sendIdVerificationResultEmail(params: {
  ...
  const subject = isApproved
    ? "Hive — Your Student ID Verification has been Approved! 🎉"
    : "Hive — Student ID Verification Status Update";
  ...
  const contentHtml = isApproved
    ? `
      ...
      <p style="font-size: 14px; color: #3f3f46; line-height: 1.6;">
        Your email address (<code>${params.email}</code>) has been whitelisted for full access to the Hive platform. You can now sign in and start collaborating on projects!
      </p>
...
export async function sendApplicationStatusEmail(params: {
  ...
  const contentHtml = isAccepted
    ...
    : `
      ...
      <p style="font-size: 14px; color: #3f3f46; line-height: 1.6;">
        The project owner has reviewed your application and decided not to proceed at this time. Don't worry — there are many other exciting open projects looking for collaborators on Hive!
      </p>
      <div style="text-align: center; margin: 30px 0;">
